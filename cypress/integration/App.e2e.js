describe('App E2E', () => {
    it('should pass happy path', () => {
        cy.visit('/');
        cy.get('[data-cy=reach-indicator]').should('have.text', 'Reach: 0 / 21+');
        cy.get('[data-cy=marquise]').click();
        cy.get('[data-cy=reach-indicator]').should('have.text', 'Reach: 10 / 21+');
        cy.get('[data-cy=alliance]').click();
        cy.get('[data-cy=reach-indicator]').should('have.text', 'Reach: 13 / 21+');
